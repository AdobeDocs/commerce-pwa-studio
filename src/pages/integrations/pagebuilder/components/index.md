---
title: Page Builder components
description: Learn about rendering Page Builder content types in PWA Studio storefront projects.
---

<Edition slots="text" backgroundColor="blue"/>
PaaS only
# Page Builder components

The Page Builder integration for PWA Studio provides React components that render the default Page Builder content types.

## List of components

- [Banner](banner/index.md)
- [Block](block/index.md)
- [Buttons and ButtonItem](buttons/index.md)
- [Column and ColumnGroup](column/index.md)
- [Divider](divider/index.md)
- [Dynamic Block](dynamic-block/index.md)
- [Heading](heading/index.md)
- [HTML Code](html-code/index.md)
- [Image](image/index.md)
- [Map](map/index.md)
- [Products](products/index.md)
- [Row](row/index.md)
- [Slider](slider/index.md)
- [Tabs and TabItem](tabs/index.md)
- [Text](text/index.md)
- [Video](video/index.md)

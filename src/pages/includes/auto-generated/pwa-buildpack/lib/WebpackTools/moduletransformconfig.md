...
Configuration builder for module transforms. Accepts `TransformRequest`  
objects and emits loader config objects for Buildpack's custom transform loaders.

Understands all transform types and normalizes them correctly. Most of this  
involves resolving file paths using Webpack or Node resolution rules.

For some special transform types, `ModuleTransformConfig` has helpers to  
apply the requested transforms itself. However, `configureWebpack` consumes  
most transforms by calling `transformConfig.collect()` on this object.  

This yields a structured object that `configureWebpack` uses to set up loader  
...
The [`configureWebpack()` function](/pwa-buildpack/reference/configure-webpack/)  
gathers `TransformRequest` objects from all interceptors of the  
`transformModules` target and turns them into a configuration of Webpack  
[module rules](https://v4.webpack.js.org/configuration/module/#modulerules).
